...
function Group({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <fieldset className="flex flex-col gap-2">
      <legend className="v2-label mb-1">{title}</legend>
      {children}
    </fieldset>
  );
}
...
function RadioGroup({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div role="radiogroup" aria-label={label} onKeyDown={onRovingKeyDown} className="flex flex-col gap-0.5">
      {children}
    </div>
  );
}
...
function Choice({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className="w-full flex items-center justify-between gap-2 px-2 h-7 text-sm text-left v2-sans"
      style={{
        borderRadius: "var(--v2-radius)",
        background: selected ? "var(--v2-panel)" : "transparent",
    ...
    >
      ...
      {count !== undefined && (
        <span className="text-xs v2-mono shrink-0" style={{ color: "var(--v2-text-faint)" }}>
          {count}
        </span>
      )}
    </button>
...
export default function FilterRail({ filters, facets, onChange }: FilterRailProps) {
  ...
  return (
    <div className="flex flex-col gap-6 v2-sans">
      <div className="flex items-center justify-between h-7">
        <p className="text-sm font-medium" style={{ color: "var(--v2-text)" }}>
          Filters
          {activeCount > 0 && (
            <span className="v2-mono" style={{ color: "var(--v2-text-faint)" }}>
              {" "}
              · {activeCount}
            </span>
          )}
          ...
          <div className="flex flex-col gap-0.5 max-h-64 overflow-y-auto -mr-1 pr-1">
            {facets.sources.map((entry) => {
              ...
              return (
                <label
                  key={entry.source}
                  className="flex items-center gap-2 px-2 h-7 text-sm cursor-pointer"
                  style={{ color: checked ? "var(--v2-text)" : "var(--v2-text-dim)" }}
                >
                  <input
                    type="checkbox"
                  ...
                  <span
                    aria-hidden
                    className="w-3.5 h-3.5 grid place-items-center shrink-0"
                    style={{
                      borderRadius: "2px",
                      border: "1px solid var(--v2-border)",
                      background: checked ? "var(--v2-accent)" : "transparent",
                      borderColor: checked ? "var(--v2-accent)" : "var(--v2-border)",
                    }}
                  >
                    {checked && <Check className="w-2.5 h-2.5" style={{ color: "var(--v2-accent-ink)" }} strokeWidth={3} />}
                  </span>
      ...
      {facets && facets.employment_types.length > 0 && (
        <Group title="Employment type">
          <RadioGroup label="Employment type">
            <Choice label="Any" selected={!filters.employmentType} onSelect={() => onChange({ employmentType: "" })} />
