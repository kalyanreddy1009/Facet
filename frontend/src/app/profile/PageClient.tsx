...
export default function ProfilePage() {
  ...
  return (
    <main className="max-w-shell mx-auto px-5 sm:px-8 py-8 sm:py-10">
      ...
      {/* Two columns past `lg`. As one stack these cards became 1100px-wide
          bands holding a 380px form, which is what "the layout falls apart on
          a large screen" looked like here. `items-start` so a short card does
          not stretch to match a tall neighbour. */}
  ...
  return (
    <section className="card p-6 sm:p-7">
      ...
      <form onSubmit={submit} className="mt-4 space-y-3 max-w-sm">
        ...
        {/* The label goes in the button, never inside `.btn-cap` — that is a
            fixed 20px disc, so a label placed in it is clipped to a circle and
            the control reads as blue text on a white dot rather than as a
            button. Same bug as the old "white dot in Sign In". */}
        <button
          type="submit"
